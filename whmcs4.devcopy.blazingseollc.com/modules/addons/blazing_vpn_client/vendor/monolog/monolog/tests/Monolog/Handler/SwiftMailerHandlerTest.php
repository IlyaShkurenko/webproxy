...
class SwiftMailerHandlerTest extends TestCase
{
    ...
    public function testMessageCreationIsLazyWhenUsingCallback()
    {
        ...
        $records = array($this->getRecord(Logger::DEBUG), $this->getRecord(Logger::INFO));
    ...
    public function testMessageHaveUniqueId()
    {
        $messageTemplate = new \Swift_Message();
        $handler = new SwiftMailerHandler($this->mailer, $messageTemplate);
        $method = new \ReflectionMethod('Blazing\\Vpn\\Client\\Vendor\\Monolog\\Handler\\SwiftMailerHandler', 'buildMessage');
