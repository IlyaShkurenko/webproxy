 ...
 */
namespace Blazing\Vpn\Client\Vendor\Monolog\Handler\Slack;
...
class SlackRecordTest extends TestCase
{
    ...
    public function dataStringify()
    {
        ...
        return array(array(array(), '[]'), array($multipleDimensions, json_encode($multipleDimensions, $jsonPrettyPrintFlag)), array($numericKeys, json_encode($numericKeys, $jsonPrettyPrintFlag)), array($singleDimension, json_encode($singleDimension)));
    }
    /**
    ...
    {
        $formatter = $this->getMock('Blazing\\Vpn\\Client\\Vendor\\Monolog\\Formatter\\FormatterInterface');
        $formatter->expects($this->any())->method('format')->will($this->returnCallback(function ($record) {
            return $record['message'] . 'test';
        }));
        $formatter2 = $this->getMock('Blazing\\Vpn\\Client\\Vendor\\Monolog\\Formatter\\FormatterInterface');
        $formatter2->expects($this->any())->method('format')->will($this->returnCallback(function ($record) {
    ...
    public function testAddsShortAttachmentWithContextAndExtra()
    {
        ...
        $this->assertSame($levelName, $attachment['title']);
        $this->assertSame(array(array('title' => 'Extra', 'value' => sprintf('```%s```', json_encode($extra, $this->jsonPrettyPrintFlag)), 'short' => false), array('title' => 'Context', 'value' => sprintf('```%s```', json_encode($context, $this->jsonPrettyPrintFlag)), 'short' => false)), $attachment['fields']);
    }
    public function testAddsLongAttachmentWithoutContextAndExtra()
    {
        ...
        $this->assertSame(array(array('title' => 'Level', 'value' => $levelName, 'short' => false)), $attachment['fields']);
    }
    public function testAddsLongAttachmentWithContextAndExtra()
    {
        ...
        $loggerRecord['extra'] = $extra;
        $data = $record->getSlackData($loggerRecord);
        $expectedFields = array(array('title' => 'Level', 'value' => $levelName, 'short' => false), array('title' => 'tags', 'value' => sprintf('```%s```', json_encode($extra['tags'])), 'short' => false), array('title' => 'test', 'value' => $context['test'], 'short' => false));
        $attachment = $data['attachments'][0];
        $this->assertArrayHasKey('title', $attachment);
        $this->assertArrayHasKey('fields', $attachment);
    ...
    {
        $record = $this->getRecord(Logger::WARNING, 'test', array('info' => array('library' => 'monolog', 'author' => 'Jordi')));
        ...
        $data = $slackRecord->getSlackData($record);
        $attachment = $data['attachments'][0];
        $expected = array(array('title' => 'info', 'value' => sprintf('```%s```', json_encode(array('author' => 'Jordi'), $this->jsonPrettyPrintFlag)), 'short' => false), array('title' => 'tags', 'value' => sprintf('```%s```', json_encode(array('web'))), 'short' => false));
