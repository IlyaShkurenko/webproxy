 ...
 */
namespace Blazing\Vpn\Client\Vendor\Monolog;

use Blazing\Vpn\Client\Vendor\Monolog\Processor\WebProcessor;
...
class LoggerTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testLog()
    {
        ...
        $handler = $this->getMock('Blazing\\Vpn\\Client\\Vendor\\Monolog\\Handler\\NullHandler', array('handle'));
    ...
    {
        $logger = new Logger(__METHOD__);
        $handler = $this->getMock('Blazing\\Vpn\\Client\\Vendor\\Monolog\\Handler\\NullHandler', array('handle'), array(Logger::ERROR));
    ...
    {
        $logger = new Logger(__METHOD__);
        $handler = $this->getMock('Blazing\\Vpn\\Client\\Vendor\\Monolog\\Handler\\HandlerInterface');
        ...
        $logger->pushHandler($handler);
        $processor = $this->getMockBuilder('Blazing\\Vpn\\Client\\Vendor\\Monolog\\Processor\\WebProcessor')->disableOriginalConstructor()->setMethods(array('__invoke'))->getMock();
    ...
    {
        $logger = new Logger(__METHOD__);
        $handler1 = $this->getMock('Blazing\\Vpn\\Client\\Vendor\\Monolog\\Handler\\HandlerInterface');
        ...
        $logger->pushHandler($handler1);
        $handler2 = $this->getMock('Blazing\\Vpn\\Client\\Vendor\\Monolog\\Handler\\HandlerInterface');
        ...
        $logger->pushHandler($handler2);
        $handler3 = $this->getMock('Blazing\\Vpn\\Client\\Vendor\\Monolog\\Handler\\HandlerInterface');
    ...
    {
        return array('with microseconds' => array(true, 'assertNotSame'), 'without microseconds' => array(false, PHP_VERSION_ID >= 70100 ? 'assertNotSame' : 'assertSame'));
    }
}