...
use Exception;
use Blazing\Vpn\Client\Vendor\Monolog\ErrorHandler;
...
class PHPConsoleHandlerTest extends TestCase
{
    ...
    public function testException()
    {
        ...
        $handler->log(\Blazing\Vpn\Client\Vendor\Psr\Log\LogLevel::ERROR, sprintf('Uncaught Exception %s: "%s" at %s line %s', get_class($e), $e->getMessage(), $e->getFile(), $e->getLine()), array('exception' => $e));
    }
    /**
    ...
    {
        return array(array('dumperLevelLimit', 'levelLimit', 1001), array('dumperItemsCountLimit', 'itemsCountLimit', 1002), array('dumperItemSizeLimit', 'itemSizeLimit', 1003), array('dumperDumpSizeLimit', 'dumpSizeLimit', 1004), array('dumperDetectCallbacks', 'detectCallbacks', true));
    }
    /**
