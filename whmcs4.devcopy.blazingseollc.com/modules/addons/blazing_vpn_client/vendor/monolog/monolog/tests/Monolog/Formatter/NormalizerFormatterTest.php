 ...
 */
namespace Blazing\Vpn\Client\Vendor\Monolog\Formatter;
...
class NormalizerFormatterTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testFormat()
    {
        ...
        $formatted = $formatter->format(array('level_name' => 'ERROR', 'channel' => 'meh', 'message' => 'foo', 'datetime' => new \DateTime(), 'extra' => array('foo' => new TestFooNorm(), 'bar' => new TestBarNorm(), 'baz' => array(), 'res' => fopen('php://memory', 'rb')), 'context' => array('foo' => 'bar', 'baz' => 'qux', 'inf' => INF, '-inf' => -INF, 'nan' => acos(4))));
        $this->assertEquals(array('level_name' => 'ERROR', 'channel' => 'meh', 'message' => 'foo', 'datetime' => date('Y-m-d'), 'extra' => array('foo' => '[object] (Monolog\\Formatter\\TestFooNorm: {"foo":"foo"})', 'bar' => '[object] (Monolog\\Formatter\\TestBarNorm: bar)', 'baz' => array(), 'res' => '[resource] (stream)'), 'context' => array('foo' => 'bar', 'baz' => 'qux', 'inf' => 'INF', '-inf' => '-INF', 'nan' => 'NaN')), $formatted);
    }
    public function testFormatExceptions()
    {
        ...
        $this->assertEquals(array('exception' => array('class' => get_class($e2), 'message' => $e2->getMessage(), 'code' => $e2->getCode(), 'file' => $e2->getFile() . ':' . $e2->getLine())), $formatted);
    }
    ...
    public function testFormatToStringExceptionHandle()
    {
        ...
        $formatter->format(array('myObject' => new TestToStringError()));
    }
    ...
    public function testNormalizeHandleLargeArrays()
    {
        ...
        $res = $formatter->format(array('level_name' => 'CRITICAL', 'channel' => 'test', 'message' => 'bar', 'context' => array($largeArray), 'datetime' => new \DateTime(), 'extra' => array()));
    ...
    public function testConvertsInvalidEncodingAsLatin9()
    {
        ...
        $reflMethod = new \ReflectionMethod($formatter, 'toJson');
        $reflMethod->setAccessible(true);
        $res = $reflMethod->invoke($formatter, array('message' => "��������"));
        if (version_compare(PHP_VERSION, '5.5.0', '>=')) {
    ...
    {
        $obj = new \stdClass();
        return array('null' => array(null, null), 'int' => array(123, 123), 'float' => array(123.45, 123.45), 'bool false' => array(false, false), 'bool true' => array(true, true), 'ascii string' => array('abcdef', 'abcdef'), 'latin9 string' => array("�1���������", '±1€ŠšŽžŒœŸÿ'), 'unicode string' => array('¤¦¨´¸¼½¾€ŠšŽžŒœŸ', '¤¦¨´¸¼½¾€ŠšŽžŒœŸ'), 'empty array' => array(array(), array()), 'array' => array(array('abcdef'), array('abcdef')), 'object' => array($obj, $obj));
    }
    /**
     * @param int    $code
     * @param string $msg
    ...
    {
        return array('depth' => array(JSON_ERROR_DEPTH, 'Maximum stack depth exceeded'), 'state' => array(JSON_ERROR_STATE_MISMATCH, 'Underflow or the modes mismatch'), 'ctrl' => array(JSON_ERROR_CTRL_CHAR, 'Unexpected control character found'), 'default' => array(-1, 'Unknown error'));
    }
