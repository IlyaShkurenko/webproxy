...
class GelfMessageFormatterTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $formatter = new GelfMessageFormatter();
        $record = array('level' => Logger::ERROR, 'level_name' => 'ERROR', 'channel' => 'meh', 'context' => array('from' => 'logger'), 'datetime' => new \DateTime("@0"), 'extra' => array('file' => 'test', 'line' => 14), 'message' => 'log');
        $message = $formatter->format($record);
        $this->assertInstanceOf('Gelf\\Message', $message);
    ...
    {
        $formatter = new GelfMessageFormatter();
        $record = array('level' => Logger::ERROR, 'level_name' => 'ERROR');
    ...
    {
        $formatter = new GelfMessageFormatter();
        $record = array('level' => Logger::ERROR, 'level_name' => 'ERROR', 'channel' => 'meh', 'context' => array('from' => 'logger', 'exception' => array('class' => '\\Exception', 'file' => '/some/file/in/dir.php:56', 'trace' => array('/some/file/1.php:23', '/some/file/2.php:3'))), 'datetime' => new \DateTime("@0"), 'extra' => array(), 'message' => 'log');
        $message = $formatter->format($record);
        $this->assertInstanceOf('Gelf\\Message', $message);
    ...
    public function testFormatWithUnlimitedLength()
    {
        ...
        $record = array('level' => Logger::ERROR, 'level_name' => 'ERROR', 'channel' => 'meh', 'context' => array('exception' => str_repeat(' ', 32767 * 2)), 'datetime' => new \DateTime("@0"), 'extra' => array('key' => str_repeat(' ', 32767 * 2)), 'message' => 'log');
        $message = $formatter->format($record);
        $messageArray = $message->toArray();
        // 200 for padding + metadata
        $length = 200;
