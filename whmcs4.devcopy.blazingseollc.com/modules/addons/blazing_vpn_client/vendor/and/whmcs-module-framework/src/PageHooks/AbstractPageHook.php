...
namespace Blazing\Vpn\Client\Vendor\WHMCS\Module\Framework\PageHooks;

use ErrorException;
use Blazing\Vpn\Client\Vendor\WHMCS\Module\Framework\Events\CallbackHook;
...
abstract class AbstractPageHook
{
    ...
    {
        if (!in_array($position, [static::POSITION_HEAD_BOTTOM, self::POSITION_BODY_TOP, static::POSITION_BODY_BOTTOM])) {
    ...
    {
        $this->codeGenerator = function () use($code) {
            return $code;
        };
        return $this;
    }
    ...
    // --- Methods to inherit
    protected abstract function getHookForPosition($position);
    // --- Processors
    ...
    protected function fixTemplate(array $vars)
    {
        ...
        switch ($template) {
            case 'viewinvoice':
                $adjustements = ['{$headoutput}' => ['marker' => preg_quote('</head>'), 'position' => 'before'], '{$headeroutput}' => ['marker' => preg_quote('<body>'), 'position' => 'after'], '{$footeroutput}' => ['marker' => preg_quote('</body>'), 'position' => 'before']];
                break;
        }
        if (!$adjustements) {
            return;
        ...
        foreach ($adjustements as $injection => $data) {
            ...
            if (false === strpos($content, $injection)) {
                $content = preg_replace("~({$data['marker']})~", 'before' == $data['position'] ? "{$injection}\n\$1" : "\$1\n{$injection}", $content, 1, $replaced);
                if (!$replaced) {
