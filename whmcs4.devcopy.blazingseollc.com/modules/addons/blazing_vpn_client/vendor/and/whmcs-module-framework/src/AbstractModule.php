...
namespace Blazing\Vpn\Client\Vendor\WHMCS\Module\Framework;
...
abstract class AbstractModule
{
    ...
    public function __construct($file, $config)
    {
        ...
        if ($id != pathinfo($directory, PATHINFO_FILENAME)) {
            throw new ErrorException("Module register: file \"{$file}\" directory name must be the same as file name, " . 'current name "' . pathinfo(realpath(dirname(__FILE__)), PATHINFO_FILENAME) . '"' . "(WHMCS requirements");
        }
        $this->id = $id;
    ...
    public function registerModuleListeners($classes = [])
    {
        foreach ($classes as $class) {
            ...
            if (!$instance instanceof $abstractParent) {
                throw new ErrorException(sprintf('Class "%s" should be inherited from "%s" class', $class, AbstractModuleListener::class));
            }
