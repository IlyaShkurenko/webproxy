...
class ArrayResult implements ArrayAccess, Iterator, Countable, \JsonSerializable
{
    ...
    public function validate($key, $message = 'Wrong response - %s (request - "%s", args - %s)', $exceptionClass = ResponseException::class)
    {
        if (!$this->isValid($key)) {
            throw new $exceptionClass(sprintf($message, json_encode($this->data), $this->apiMethod, json_encode($this->apiArgs)));
        }
        return $this;
    }
    // Accessors
