...
use Blazing\Vpn\Client\Vendor\WHMCS\Module\Framework\ConfigBuilder\AddonConfigBuilder;
class Addon extends AbstractModule
{
    ...
    public static function isModuleEnabled($id)
    {
        return self::getStaticCachedResult(static::TYPE . $id, function () use($id) {
            $result = !!Helper::conn()->selectOne("SELECT count(*) as enabled FROM tbladdonmodules WHERE module = ?", [$id])['enabled'];
            Helper::restoreDb();
            return $result;
        });
    }
