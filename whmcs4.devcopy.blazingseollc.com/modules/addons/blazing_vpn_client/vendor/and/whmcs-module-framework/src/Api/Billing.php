...
class Billing extends AbstractRequest
{
    ...
    {
        return $this->response('addCredit', ['clientId' => $userId, 'amount' => $amount, 'description' => $description]);
    }
    public function addInvoicePayment($invoiceId, $transactionId, $gateway, $amount = null, $date = null, array $args = [])
    {
        $args = array_merge(['invoiceId' => $invoiceId, 'transId' => $transactionId, 'gateway' => $gateway], $args);
        if ($amount) {
            $args['amount'] = $amount;
        }
        if ($date) {
    ...
    public function addInTransaction($userId, $paymentMethod, $sum, $description = null, $invoiceId = null, array $args = [])
    {
        $args = array_merge(['amountin' => $sum], $args);
        return $this->addTransaction($userId, $paymentMethod, $description, $invoiceId, $args);
    }
    public function addOutTransaction($userId, $paymentMethod, $sum, $description = null, $invoiceId = null, array $args = [])
    {
        $args = array_merge(['amountout' => $sum], $args);
        return $this->addTransaction($userId, $paymentMethod, $description, $invoiceId, $args);
    }
    public function addTransaction($userId, $paymentMethod, $description = null, $invoiceId = null, array $args = [])
    {
        $args = array_merge(['paymentmethod' => $paymentMethod, 'userid' => $userId], $args);
        if (null !== $description) {
