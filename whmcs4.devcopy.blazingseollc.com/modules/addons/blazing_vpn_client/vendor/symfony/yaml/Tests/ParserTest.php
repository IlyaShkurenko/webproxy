...
use Blazing\Vpn\Client\Vendor\Symfony\Component\Yaml\Parser;
use Blazing\Vpn\Client\Vendor\Symfony\Component\Yaml\Tag\TaggedValue;
...
- bar: |
    ...
    two
EOF;
        $expected = array('foo' => array(array('bar' => "one\n\ntwo")));
...
YAML;
    ...
    {
        $yaml = "items:  \n  foo: bar";
        $expected = array('items' => array('foo' => 'bar'));
        $this->assertSame($expected, $this->parser->parse($yaml));
    }
    /**
     * @group legacy
...
bar: 1
EOF;
        return array('yaml-tag' => array($yamlTag), 'local-tag' => array($localTag));
    }
    /**
    ...
    public function testNonUtf8Exception()
    {
        ...
        foreach ($yamls as $yaml) {
            try {
                ...
                $this->assertInstanceOf('Blazing\\Vpn\\Client\\Vendor\\Symfony\\Component\\Yaml\\Exception\\ParseException', $e, 'charsets other than UTF-8 are rejected.');
            }
        }
    }
    /**
...
d: e
EOT;
        $expected = array('a' => array(array('b' => array(array('bar' => 'baz'))), 'foo'), 'd' => 'e');
        $this->assertSame($expected, $this->parser->parse($yaml));
    }
...
parent:
    ...
    child: duplicate
EOD;
        $expected = array('parent' => array('child' => 'first'));
...
EOF;
    ...
    {
        $this->assertEquals(array('services' => array('app.foo_service' => array('class' => 'Foo'), 'app/bar_service' => array('class' => 'Bar'))), Yaml::parse(<<<'EOF'
# comment 1
services:
# comment 2
    # comment 3
...
EOF
    ...
    {
        $this->assertEquals(array(array('title' => 'some title', 'content' => <<<'EOT'
# comment 1
header
...
foo:
    ...
    1.3: "baz"
EOF;
        $expected = array('foo' => array('1.2' => 'bar', '1.3' => 'baz'));
...
false: bar
EOF;
        $expected = array('true' => 'foo', 'false' => 'bar');
        $this->assertEquals($expected, $this->parser->parse($yaml, Yaml::PARSE_KEYS_AS_STRINGS));
    }
    /**
     * @expectedException \Symfony\Component\Yaml\Exception\ParseException
...
pages:
    -
        ...
        content: |
            ...
            footer # comment3
EOT;
        $expected = array('pages' => array(array('title' => 'some title', 'content' => <<<'EOT'
# comment 1
header
...
collection:
    ...
    - two: |
        ...
        baz
EOT;
        $expected = array('test' => <<<'EOT'
foo
# bar
baz

EOT
, 'collection' => array(array('one' => <<<'EOT'
foo
# bar
baz
...
# comment
    foobar: ~
EOT;
        $expected = array('foo' => array('bar' => array('scalar-block' => "line1 line2>\n"), 'baz' => array('foobar' => null)));
        $tests[] = array($yaml, $expected);
        $yaml = <<<'EOT'
a:
    b: hello
...
#        second row
    d: hello
EOT;
        $expected = array('a' => array('b' => 'hello', 'd' => 'hello'));
        $tests[] = array($yaml, $expected);
        return $tests;
    }
...
EOT;
        $this->assertSame(array('test' => <<<'EOT'
<h2>A heading</h2>
...
EOT
    ...
    {
        return array('enclosed with double quotes' => array('data: !!binary "SGVsbG8gd29ybGQ="'), 'enclosed with single quotes' => array("data: !!binary 'SGVsbG8gd29ybGQ='"), 'containing spaces' => array('data: !!binary  "SGVs bG8gd 29ybGQ="'), 'in block scalar' => array(<<<'EOT'
data: !!binary |
    SGVsbG8gd29ybGQ=
EOT
), 'containing spaces in block scalar' => array(<<<'EOT'
data: !!binary |
    SGVs bG8gd 29ybGQ=
EOT
));
...
EOT;
    ...
    {
        if (method_exists($this, 'expectException')) {
            $this->expectException('Blazing\\Vpn\\Client\\Vendor\\Symfony\\Component\\Yaml\\Exception\\ParseException');
            $this->expectExceptionMessage(sprintf('Unexpected characters near "," at line %d (near "bar: "123",").', $lineNumber));
        } else {
            $this->setExpectedException('Blazing\\Vpn\\Client\\Vendor\\Symfony\\Component\\Yaml\\Exception\\ParseException', sprintf('Unexpected characters near "," at line %d (near "bar: "123",").', $lineNumber));
        }
        $this->parser->parse($yaml);
    }
    public function parserThrowsExceptionWithCorrectLineNumberProvider()
    {
        return array(array(4, <<<'YAML'
foo:
    -
        # bar
        bar: "123",
...
EOF;
    ...
    {
        return array('sequences' => array(array(new TaggedValue('foo', array('yaml')), new TaggedValue('quz', array('bar'))), <<<YAML
- !foo
    - yaml
- !quz [bar]
YAML
), 'mappings' => array(new TaggedValue('foo', array('foo' => new TaggedValue('quz', array('bar')), 'quz' => new TaggedValue('foo', array('quz' => 'bar')))), <<<YAML
!foo
foo: !quz [bar]
quz: !foo
   quz: bar
YAML
), 'inline' => array(array(new TaggedValue('foo', array('foo', 'bar')), new TaggedValue('quz', array('foo' => 'bar', 'quz' => new TaggedValue('bar', array('one' => 'bar'))))), <<<YAML
- !foo [foo, bar]
...
YAML;
        $expected = array('transitions' => array('foo' => array('from' => array('bar'), 'to' => 'baz')));
...
foobar:
    ...
    <<: [*FOO, *BAR]
YAML;
        $expected = (object) array('foo' => (object) array('bar' => 1), 'bar' => (object) array('baz' => 2, 'bar' => 1), 'baz' => (object) array('baz_foo' => 3, 'baz_bar' => 4), 'foobar' => (object) array('bar' => null, 'baz' => 2));
...
mergekeyderef:
    ...
    <<: *quux
YAML;
        $expected = array('mergekeyrefdef' => array('a' => 'foo', 'b' => 'bar', 'c' => 'baz'), 'mergekeyderef' => array('d' => 'quux', 'b' => 'bar', 'c' => 'baz'));
        $this->assertSame($expected, $this->parser->parse($yaml));
    }
}
class B
