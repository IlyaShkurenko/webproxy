 ...
 */
namespace Blazing\Vpn\Client\Vendor\Symfony\Component\Yaml;
        ...
        do {
            ...
            if (self::preg_match('#^\\-((?P<leadspaces>\\s+)(?P<value>.+))?$#u', rtrim($this->currentLine), $values)) {
                ...
                if (!isset($values['value']) || '' == trim($values['value'], ' ') || 0 === strpos(ltrim($values['value'], ' '), '#')) {
                    ...
                    if (isset($values['leadspaces']) && self::preg_match('#^(?P<key>' . Inline::REGEX_QUOTED_STRING . '|[^ \'"\\{\\[].*?) *\\:(\\s+(?P<value>.+?))?\\s*$#u', $this->trimTag($values['value']), $matches)) {
            ...
            } elseif (self::preg_match('#^(?P<key>' . Inline::REGEX_QUOTED_STRING . '|(?:!?!php/const:)?(?:![^\\s]++\\s++)?[^ \'"\\[\\{!].*?) *\\:(\\s++(?P<value>.+))?$#u', rtrim($this->currentLine), $values) && (false === strpos($values['key'], ' #') || in_array($values['key'][0], array('"', "'")))) {
