 ...
 */
namespace Blazing\Vpn\Client\Vendor\Symfony\Component\HttpFoundation\Session\Storage\Handler;
...
class MongoDbSessionHandler implements \SessionHandlerInterface
{
    ...
    public function __construct($mongo, array $options)
    {
        ...
        $this->mongo = $mongo;
        $this->options = array_merge(array('id_field' => '_id', 'data_field' => 'data', 'time_field' => 'time', 'expiry_field' => 'expires_at'), $options);
    }
    /**
     * {@inheritdoc}
     */
    ...
    public function destroy($sessionId)
    {
        ...
        $this->getCollection()->{$methodName}(array($this->options['id_field'] => $sessionId));
        return true;
    }
    /**
     * {@inheritdoc}
    ...
    public function gc($maxlifetime)
    {
        ...
        $this->getCollection()->{$methodName}(array($this->options['expiry_field'] => array('$lt' => $this->createDateTime())));
        return true;
    }
    /**
     * {@inheritdoc}
    ...
    public function write($sessionId, $data)
    {
        ...
        $fields = array($this->options['time_field'] => $this->createDateTime(), $this->options['expiry_field'] => $expiry);
        ...
        $this->getCollection()->{$methodName}(array($this->options['id_field'] => $sessionId), array('$set' => $fields), $options);
        return true;
    }
    /**
     * {@inheritdoc}
    ...
    {
        $dbData = $this->getCollection()->findOne(array($this->options['id_field'] => $sessionId, $this->options['expiry_field'] => array('$gte' => $this->createDateTime())));
        if (null === $dbData) {
            return '';
        }
