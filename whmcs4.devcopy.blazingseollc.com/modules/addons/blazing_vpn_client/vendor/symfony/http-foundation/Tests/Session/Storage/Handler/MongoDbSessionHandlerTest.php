 ...
 */
namespace Blazing\Vpn\Client\Vendor\Symfony\Component\HttpFoundation\Tests\Session\Storage\Handler;

use PHPUnit\Framework\TestCase;
use Blazing\Vpn\Client\Vendor\Symfony\Component\HttpFoundation\Session\Storage\Handler\MongoDbSessionHandler;
/**
...
class MongoDbSessionHandlerTest extends TestCase
{
    ...
    protected function setUp()
    {
        ...
        $this->options = array('id_field' => '_id', 'data_field' => 'data', 'time_field' => 'time', 'expiry_field' => 'expires_at', 'database' => 'sf2-test', 'collection' => 'session-test');
    ...
    public function testRead()
    {
        ...
        $testTimeout = time() + 1;
        $collection->expects($this->once())->method('findOne')->will($this->returnCallback(function ($criteria) use($testTimeout) {
            ...
            $fields = array($this->options['id_field'] => 'foo');
            if (phpversion('mongodb')) {
    ...
    public function testWrite()
    {
        ...
        $collection->expects($this->once())->method($methodName)->will($this->returnCallback(function ($criteria, $updateData, $options) use(&$data) {
    ...
    {
        $this->options = array('id_field' => '_id', 'data_field' => 'data', 'time_field' => 'time', 'database' => 'sf2-test', 'collection' => 'session-test', 'expiry_field' => 'expiresAt');
        $this->storage = new MongoDbSessionHandler($this->mongo, $this->options);
        $collection = $this->createMongoCollectionMock();
        $this->mongo->expects($this->once())->method('selectCollection')->with($this->options['database'], $this->options['collection'])->will($this->returnValue($collection));
        $data = array();
    ...
    public function testReplaceSessionData()
    {
        ...
        $data = array();
        $methodName = phpversion('mongodb') ? 'updateOne' : 'update';
        $collection->expects($this->exactly(2))->method($methodName)->will($this->returnCallback(function ($criteria, $updateData, $options) use(&$data) {
            $data = $updateData;
        }));
    ...
    public function testDestroy()
    {
        ...
        $collection->expects($this->once())->method($methodName)->with(array($this->options['id_field'] => 'foo'));
    ...
    public function testGc()
    {
        ...
        $collection->expects($this->once())->method($methodName)->will($this->returnCallback(function ($criteria) {
            if (phpversion('mongodb')) {
    ...
    private function createMongoCollectionMock()
    {
        ...
        $collection = $this->getMockBuilder($collectionClass)->disableOriginalConstructor()->getMock();
        return $collection;
    }
}